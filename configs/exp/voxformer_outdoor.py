# VoxFormer-style occupancy prediction for indoor dataset
#
# Based on COTR indoor config (same backbone pipeline).
# Replaces COTR's MaskFormer head with VoxFormer-style:
#   - Binary occupancy proposal
#   - Proposal-guided feature enhancement
#   - 3D CNN completion network
#   - Per-voxel CE + geo_scal + sem_scal losses
...
model = dict(
    ...
    bev_encoder_neck=dict(
        ...
        reverse=True,
        size=(24, 50, 50)),
    # ---- VoxFormer-style head ----
    occupancy_head=dict(
        type='VoxFormerOccHead',
        in_channels=numC_Trans,       # 32, from BEV encoder neck
        embed_dims=64,                # intermediate feature dims
        num_classes=num_classes,       # 21
        mid_channels=64,              # completion network channels
        CE_ssc_loss=True,
        geo_scal_loss=True,
        sem_scal_loss=True,
        loss_ce_weight=10.0,
