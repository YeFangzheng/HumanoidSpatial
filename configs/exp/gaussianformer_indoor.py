...
#
# 室内 GaussianFormer：官方 OpenOcc / mmseg ``BEVSegmentor`` 配置（vendored 源码在
# ``mmdet3d/models/GaussianFormer``）。勿用 ``tools/train.py`` 跑本文件。
#
# 官方权重 + RayMetric 评测（与 MMDet 室内 RayIoU 对齐）。``torchrun --nproc_per_node>1`` 时
# 仅 ``LOCAL_RANK==0`` 跑评测，其余进程立即退出（本脚本不启 DDP；多卡并行推理需另做数据并行）::
#
#   cd /path/to/Occupancy_Giga-benchmark
#   export PYTHONPATH=$PWD:$PYTHONPATH
#   torchrun --nproc_per_node=4 tools/test.py configs/exp/gaussianformer_indoor.py \\
#     output/gaussianformer_indoor/epoch_10.pth --launcher pytorch --work-dir output_test/test
#
# 单卡::
#
#   python tools/test.py configs/exp/gaussianformer_indoor.py \\
#     output/gaussianformer_indoor/epoch_10.pth --work-dir output_test/test
#
...
#     --work-dir output/gaussianformer_indoor
#
...
warmup_iters = 1000
min_lr_ratio = 0.05
# 官方 train.py 内置 MeanIoU 写死 nuScenes 类别，对室内无意义；用 Occ-benchmark ``tools/test.py`` + RayMetric。
