...
# 以现有 SurroundOcc 模块“瓶装”OccFormer风格配置：
# 使用统一隐藏维度、更深encoder层数、更大采样点数。
occ_size = [200, 200, 24]
_embed_dims_ = [256, 256, 256]
_ffn_dim_ = [512, 512, 512]
_num_points_ = [8, 8, 8]
_num_layers_ = [2, 4, 6]
volume_h_ = [100, 50, 25]
volume_w_ = [100, 50, 25]
...
model = dict(
    ...
    occupancy_head=dict(
        ...
        conv_input=[_embed_dims_[2], 256, _embed_dims_[1], 128, _embed_dims_[0], 64, 64],
        conv_output=[256, _embed_dims_[1], 128, _embed_dims_[0], 64, 64, 32],
        out_indices=[0, 2, 4, 6],
...
test_pipeline = [
    ...
    dict(
        ...
        keys=['img', 'points', 'gt_occupancy'],
        meta_keys=['lidar_origins', 'visible_mask'] + base_metas,
    ),
]
...
custom_imports = dict(
    imports=['mmdet3d.models.surroundocc'],
