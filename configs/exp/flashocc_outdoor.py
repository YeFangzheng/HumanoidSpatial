...
# This config is adapted from FBOcc for RoboPanda XHumanoid occupancy learning.
# NOTE: it uses registry-safe module names available in the current codebase.
...
# Model
# FlashOcc generally follows camera-to-BEV occupancy flow but uses lighter/faster
# lifting + BEV encoder/head components.
grid_config = {
    'x': [-10, 10, 0.2],
    'y': [-10, 10, 0.2],
    'z': [-1.5, 0.9, 0.2],
...
data_root = '$PATH_TO_DATASET$/Data_outdoor'
file_client_args = dict(backend='disk')
occupancy_path = '$PATH_TO_DATASET$/Data_outdoor/annotation/occ'

base_metas = [
    'ori_img', 'scene_token', 'scene_name', 'frame_idx', 'prev_exists',
    'intrinsic', 'distortion', 'cam2ego', 'post_trans', 'bda',
    'ego_pose', 'ego_pose_inv', 'timestamp'
]

train_pipeline = [
    dict(type='PrepareImageInputs', is_train=True, data_config=data_config, undistort=True),
    dict(type='LoadPointsFromPCD', coord_type='LIDAR', load_dim=4, use_dim=4),
    dict(type='PointToMultiViewDepth', downsample=1, grid_config=grid_config),
    dict(type='LoadOccupancyXHumanoid', occupancy_path=occupancy_path, class_names=class_names),
    dict(
        type='RemapOccLabels',
...
param_scheduler = [
    ...
    dict(type='CosineAnnealingLR', begin=0, T_max=num_epochs, end=num_epochs, by_epoch=True, eta_min=1e-5),
]
...
custom_hooks = [dict(type='SequentialControlHook', temporal_start_epoch=2)]
