...
data_root = '$PATH_TO_DATASET$/Data_indoor'  # <-- 修改为你的数据根目录
...
def process_clip(clip_info):
    try:
        ...
        for token in frames:
            ...
            for kitti_bbox in kitti_bboxes:
                ...
                if kitti_bbox[0] == 'pedestrian':
                    in_box = np.logical_and(in_box, seg_labels == 1)  # 站立行人
                elif kitti_bbox[0] == 'irregular_pedestrian':
                    in_box = np.logical_and(in_box, seg_labels == 2)  # 坐/躺等特殊姿态
        ...
        for token in frames:
            ...
            labels[labels == 12] = 255  # others -> unknown

            # OCC 生成时过滤掉 unknown(255)，使其不参与体素投票/不产生占据
            keep_mask = (labels != 255)
            ...
            # === 画/导出时明确不画 255（unknown）===
            vis_mask = (processed_label > 0) & (processed_label != 255)
