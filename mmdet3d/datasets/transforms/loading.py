...
class LoadPointsFromFile(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        points.translate(lidar2ego[:3, 3])
...
class PrepareImageInputs(object):
    ...
    def __init__(
        ...
        self.shoulder_aug = shoulder_aug
    ...
    def __call__(self, results):
        ...
        for cam_name in cam_names:
            cam_data = results['images'][cam_name]
            ...
            cam2img = np.array(cam_data['cam2img'])
            if self.undistort:
                dist = np.array(list(cam_data['distortion'].values()))
                img = cv.undistort(img, cam2img, dist)
            if self.shoulder_aug and np.random.uniform() < 0.3:
                if 'LEFT' in cam_name or 'RIGHT' in cam_name:
                    shoulder = cv.imread(f'mask/{cam_name}_shoulder_ori.png')
                    mask = (shoulder > 0).any(-1)
                    img[mask] = shoulder[mask]
            ori_imgs.append(torch.tensor(img))
            intrin = torch.Tensor(cam_data['cam2img'])
            cam2ego = torch.Tensor(cam_data['cam2ego'])
            distortion = torch.Tensor(list(cam_data['distortion'].values()))
            ...
            img_augs = self.sample_augmentation(
                H=img.height, W=img.width, flip=flip)
            ...
            post_tran = torch.eye(4)
            post_tran[:2, 3] = post_tran2
            post_tran[:2, :2] = post_rot2
            ...
            if self.sequential:
                ...
                for adj_info in results['adjacent']:
                    filename_adj = adj_info['images'][cam_name]['data_path']
        ...
        if self.sequential:
            for adj_info in results['adjacent']:
                ...
                cam2egos_adj = []
                for cam_name in cam_names:
                    cam_data = adj_info['images'][cam_name]
                    intrin = torch.Tensor(cam_data['cam2img'])
                    cam2ego = torch.Tensor(cam_data['cam2ego'])
                    cam2egos_adj.append(cam2ego)

                cam2egos.extend(cam2egos_adj)
        imgs = torch.stack(imgs)
        ori_imgs = torch.stack(ori_imgs)
        cam2egos = torch.stack(cam2egos)
        ...
        results['intrinsic'] = intrins
        results['distortion'] = distortions
        results['cam2ego'] = cam2egos
...
class BDA(object):
    ...
    def __call__(self, results):
        ...
        results['bda'] = bda_rot
        results['ego2global'] = torch.Tensor(results['ego2global'])
        results['ego_pose'] = results['ego2global'] @ torch.inverse(bda_mat)
        results['ego_pose_inv'] = bda_mat @ torch.inverse(results['ego2global'])
        ...
        ### bda for occupancy & points
        if 'gt_occupancy' in results:
            occupancy = results['gt_occupancy']
            if rotate_bda != 0:
                occupancy = occupancy.permute(2, 0, 1)
                occupancy = rotate(occupancy, -rotate_bda, fill=255).permute(1, 2, 0)
            if flip_dx:
        ...
        if 'points' in results:
            points = results['points']
            if rotate_bda != 0:
                points.rotate(rotate_bda / 180 * np.pi)
            if flip_dx:
                points.flip('vertical') # NOTE
            if flip_dy:
        ...
        if 'voxel_semantics' in results:
            ...
            final_instances = results['voxel_instances']
            if rotate_bda != 0:
                semantics = semantics.permute(2, 0, 1)  # [16, 200, 200]
                semantics = rotate(semantics, rotate_bda, fill=255).permute(1, 2, 0)  # [200, 200, 16]

                final_instances = final_instances.permute(2, 0, 1)  # [16, 200, 200]
                final_instances = rotate(final_instances, rotate_bda, fill=255).permute(1, 2, 0)  # [200, 200, 16]
            if flip_dx:
                semantics = torch.flip(semantics, [1])
                final_instances = torch.flip(final_instances, [1])
            if flip_dy:
                semantics = torch.flip(semantics, [0])
                final_instances = torch.flip(final_instances, [0])
...
class LoadOccupancy(object):
    ...
    def __call__(self, results):
        ...
        occupancy_file_path = os.path.join(self.occupancy_path, scene_name, sample_token, 'labels.npz')
        data = np.load(occupancy_file_path)
        occupancy = torch.tensor(data['semantics'])
        visible_mask = torch.tensor(data[self.mask]).bool()
        ...
        visible_mask = torch.rot90(visible_mask, 1, [1, 2])
        visible_mask = torch.flip(visible_mask, [1])
        visible_mask = visible_mask.permute(1, 2, 0)
...
@TRANSFORMS.register_module()
class LoadOccupancyXHumanoid(object):
    """Load an image from file in monocular 3D object detection. Compared to 2D
    detection, additional camera parameters need to be loaded.
    ...
    def __init__(self, ignore_classes=[], occupancy_path=None, class_mapping=None, class_names=None, range=None):
        self.ignore_classes = ignore_classes
        self.occupancy_path = occupancy_path
    ...
    def __call__(self, results):
        ...
        occupancy_file_path = os.path.join(self.occupancy_path, f"{results['scene_token']}/{os.path.basename(results['lidar_points']['lidar_path']).replace('pcd', 'npz')}")
        occupancy = np.load(occupancy_file_path)['occ'][:,:,:24]
        if self.range:
            occupancy = occupancy[self.range[0]: self.range[1], self.range[2]: self.range[3]]
        occupancy = torch.tensor(occupancy)

        # class mapping
        if self.class_mapping is not None:
            for i, key in enumerate(self.class_mapping.keys()):
                occupancy[occupancy == i] = self.class_names.index(self.class_mapping[key])
...
@TRANSFORMS.register_module()
class LoadSparseOcc(LoadOccupancyXHumanoid):
    ...
    def __call__(self, results):
        ...
        semantics = occupancy.clone()
        semantics[semantics < 255] -= 1
        semantics[semantics == -1] = self.num_classes - 1
        instances = None
        instance_class_ids = None

        instance_count = 0
        final_instance_class_ids = []
        final_instances = torch.ones_like(semantics) * 255  # empty space has instance id "255"

        for class_id in range(self.num_classes - 1):
            if torch.sum(semantics == class_id) == 0:
                continue
        ...
        final_instance_class_ids = torch.tensor(final_instance_class_ids)

        results['voxel_semantics'] = semantics
        results['voxel_instances'] = final_instances
        results['instance_class_ids'] = final_instance_class_ids
...
class PointToMultiViewDepth(object):
    ...
    def __call__(self, results):
        ...
        depth_map_list = []
        for i in range(len(cam2ego)):
            cam2img = np.eye(4, dtype=np.float32)
            cam2img = torch.from_numpy(cam2img)
            cam2img[:3, :3] = intrinsic[i]

            points2img = cam2img.matmul(torch.inverse(cam2ego[i]))
            points_img = points_lidar.tensor[:, :3].matmul(
                points2img[:3, :3].T) + points2img[:3, 3].unsqueeze(0)
            points_img = torch.cat(
                [points_img[:, :2] / points_img[:, 2:3], points_img[:, 2:3]],
                1)
            points_img = points_img.matmul(
                post_trans[i, :3, :3].T) + post_trans[i, :3, 3]
...
@TRANSFORMS.register_module()
class RemapOccLabels(object):
    """Remap occupancy labels in-place.
    Typical use: map some labels to ignore_index (e.g., 255) so they don't
    contribute to loss, and this works for all occ models.
    """

    def __init__(self, mapping: dict, ignore_index: int = 255, keys=('gt_occupancy',)):
    ...
    def __call__(self, results: dict):
        for key in self.keys:
            ...
            # support torch.Tensor / numpy.ndarray
            if isinstance(occ, torch.Tensor):
                # ensure integer tensor
                if not occ.dtype.is_floating_point:
                    pass
                else:
                    occ = occ.to(torch.long)
