...
class Voxelization(Function):
    ...
    def forward(
        ...
        if max_points == -1 or max_voxels == -1:
            ...
            voxelize_ext.dynamic_voxelize_forward(
                points,
            ...
            voxelize_ext.hard_voxelize_forward(
                points,
                torch.tensor(voxel_size, dtype=torch.float32),
                torch.tensor(coors_range, dtype=torch.float32),
                voxels,
...
class VoxelizationByGridShape(nn.Module):
    ...
    def forward(self, input: torch.Tensor, padding=False) -> torch.Tensor:
        if self.training:
            max_voxels = self.max_voxels[0]
        else:
            max_voxels = self.max_voxels[1]

        voxels, coors, num_points_per_voxel, voxel_num = voxelization(input, self.voxel_size, self.point_cloud_range,
...
class _DynamicScatter(Function):
    ...
    def forward(ctx: Any,
        ...
        """
        results = voxelize_ext.dynamic_point_to_voxel_forward(
    ...
    def backward(ctx: Any,
        ...
        voxelize_ext.dynamic_point_to_voxel_backward(
