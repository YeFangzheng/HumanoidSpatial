...
class SyncBatchNormFunction(Function):
    ...
    def forward(self, input: torch.Tensor, running_mean: torch.Tensor,
        ...
        if batch_size > 0:
            sync_bn_ext.sync_bn_forward_mean(input3d, mean)
        ...
        if batch_size > 0:
            sync_bn_ext.sync_bn_forward_var(input3d, mean, var)
        ...
        sync_bn_ext.sync_bn_forward_output(
            input3d,
            mean,
            var,
            weight,
    ...
    def backward(self, grad_output: torch.Tensor) -> tuple:
        ...
        if batch_size > 0:
            sync_bn_ext.sync_bn_backward_param(grad_output3d, norm, grad_weight,
                                              grad_bias)
        ...
        if batch_size > 0:
            sync_bn_ext.sync_bn_backward_data(grad_output3d, weight,
