...
class COTRHead(BaseModule):
    ...
    def __init__(self,
        ...
        self.transformer = MODELS.build(transformer) if transformer is not None else None
    ...
    def loss(self,
        ...
        for group_index in range(self.group_detr):
            ...
            # self.loss_cls.class_weight = [0.1] + [1.0]*self.group_classes[group_index] # 0 as free
            losses_cls, losses_mask, losses_dice = multi_apply(
                self.loss_single, group_cls_scores, group_mask_scores, group_gt_classes_list,
                group_gt_masks_list, all_img_metas_list)
            # loss from last decoder layer
    ...
    def merge(self, pred1, pred2):
        """pred1: decoder mask-based result, pred2: encoder per-voxel result"""
        result = pred2.clone()
        pred1 = pred1.to(result.dtype)
        decoder_valid = (pred1 != self.num_classes) & (pred1 != 0)
        result[decoder_valid] = pred1[decoder_valid]
        return result
