...
class HungarianMatcher(BaseModule):
    ...
    def __init__(self, cost_class: float = 1, cost_mask: float = 1, cost_dice: float = 1):
        ...
        self.loss_focal = TASK_UTILS.build(dict(type='mmdet.FocalLossCost', weight=2.0))
    ...
    def forward(self, mask_pred, class_pred, mask_gt, class_gt, mask_camera):
        ...
        for b in range(bs):
            ...
            pred_instances.scores = out_prob
            gt_instances = InstanceData()
            gt_instances.labels = tgt_ids.long()
            cost_class = self.loss_focal(pred_instances, gt_instances)
