...
def get_voxel_decoder_loss_input(voxel_semantics, occ_loc_i, seg_pred_i, scale, num_classes=18):
    ...
    if seg_pred_i is not None:  # semantic prediction
        ...
        seg_pred_dense, sparse_mask = sparse2dense(
            ...
            dense_shape=[200 // scale, 200 // scale, 24 // scale, num_classes],
...
class SparseOccHead(nn.Module): 
    def __init__(self,
        ...
        if balance_cls_weight:
            self.class_weights = torch.from_numpy(1 / np.log(humanoid_industry_frequencies[:self.num_classes] + 0.001))
        else:
    ...
    def loss_single(self, voxel_semantics, voxel_instances, instance_class_ids, preds_dicts, mask_camera=None):
        ...
        for i, (occ_loc_i, _, seg_pred_i, _, scale) in enumerate(preds_dicts['occ_preds']):
            ...
            for b in range(B):
                ...
                loss_dict_i_b['loss_sem_lovasz'] = lovasz_softmax(torch.softmax(seg_pred_i_sparse, dim=1), voxel_semantics_sparse, ignore=255)
        ...
        for b in range(B):
            ...
            temp_voxel.append(id_map[voxel_instances[b].long()])
            ...
            test_instance.append(instance_class_ids[b][instance_voxel_counts[:instance_count] > 0])
        temp_voxel = torch.stack(temp_voxel)

        for i, pred in enumerate(preds_dicts['mask_preds']):
            indices = self.matcher(pred, preds_dicts['class_preds'][i], temp_voxel, test_instance, mask_camera)
            loss_mask, loss_dice, loss_class = self.criterions['loss_mask2former'](
                pred, preds_dicts['class_preds'][i], temp_voxel, test_instance, indices, mask_camera)
        ...
        if cur_masks.shape[0] != 0:
            ...
            for k in range(cur_classes.shape[0]):
                ...
                is_thing = self.class_names[pred_class] in [
                    'pedestrian', 'animal', 'bicycle', 'cyclist', 
                    'tricycle', 'vehicle',
                ]
