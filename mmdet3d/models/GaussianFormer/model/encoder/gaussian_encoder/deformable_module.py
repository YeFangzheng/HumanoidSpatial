...
try:
    ...
    DAF = None
# Occ-benchmark: reuse mmdet3d CUDA ext when vendored ``ops/`` is not pip-installed.
if DAF is None:
    try:
        from mmdet3d.ops.deformable_aggregation import (
...
class DeformableFeatureAggregation(BaseModule):
    def __init__(
        ...
        if use_deformable_func and DAF is None:
            raise RuntimeError(
                'DeformableAggregationFunction 未加载（CUDA 扩展）。请任选其一：'
                '(1) cd mmdet3d/models/GaussianFormer/model/encoder/gaussian_encoder/ops '
                '&& pip install -e . ；'
                '(2) 确保本仓库 mmdet3d 的 deformable_aggregation 已编译安装（与 MMDet 训练共用）。')
