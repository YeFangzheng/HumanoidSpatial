...
def main(local_rank, args):
    ...
    # init DDP — ``torchrun`` / ``train_gaussianformer_official`` sets env; legacy spawn uses tcp.
    use_torchrun = getattr(args, 'distributed_backend', '') == 'torchrun'
    if use_torchrun and args.gpus > 1:
        distributed = True
    ...
    grad_norm = 0
    skip_builtin_eval = cfg.get('skip_builtin_eval', False)
    if not skip_builtin_eval:
    ...
    while epoch < max_num_epochs:
        ...
        loss_list = []
        _sleep = float(cfg.get('trainer_sleep_s', 10))
        if _sleep > 0:
            time.sleep(_sleep)
        ...
        # eval (nuScenes mIoU; skip when ``skip_builtin_eval`` — use Occ-benchmark test instead)
        if skip_builtin_eval or epoch % cfg.get('eval_every_epochs', 1) != 0:
            continue
        my_model.eval()
